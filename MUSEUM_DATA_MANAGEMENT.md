...
MuseumCheck 使用灵活的三级数据管理架构，支持开发调试和稳定发布。该系统允许开发者在远程存储中快速迭代内容，稳定后发布为静态文件，同时保证应用离线可用。
...
1. **首次加载**：根据配置的优先级顺序尝试加载
2. **缓存机制**：成功加载的数据会缓存在内存中
3. **降级策略**：当前层级失败时自动切换到下一优先级
4. **智能选择**：用户可配置三种优先级模式
...
**配置**：
```javascript
museumDataLoader.updatePrioritySettings(['tier1', 'tier2', 'tier3']);
```
