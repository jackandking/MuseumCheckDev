...
check_file_placement() {
    ...
    case "$file" in
        ...
        *.html|*.js|*.css)
            # 只允许主要应用文件在根目录
            if [[ "$file" =~ ^(pilot|museum-checkin|achievements|treasures|settings|fireworks|event-wall|everyone-achievements|leaderboard|fireworks-wall|together|museum-live)\.html$ ]]; then
                return 0
            fi
            # 其他HTML文件应该在子目录
            if [ "$dirname" = "." ]; then
